...
static bool collisionBoxToBox(ci::Vec3f box_pos1, ci::Vec3f box_size1, ci::Vec3f box_pos2, ci::Vec3f box_size2) {
	...
	if (box_pos2.x >= box_pos1.x + box_size1.x) return false;
	if (box_pos2.x + box_size2.x <= box_pos1.x) return false;
	if (box_pos2.y >= box_pos1.y + box_size1.y) return false;
	if (box_pos2.y + box_size2.y <= box_pos1.y) return false;
	if (box_pos2.z >= box_pos1.z + box_size1.z) return false;
	if (box_pos2.z + box_size2.z <= box_pos1.z) return false;
...
static bool returnBoxToRayPoint(ci::Vec3f box_pos, ci::Vec3f box_size, ci::Vec3f ray_point) {
	box_pos -= box_size / 2;
	if (box_pos.x > ray_point.x) return false;
	if (box_pos.x + box_size.x < ray_point.x) return false;
	if (box_pos.y > ray_point.y) return false;
	if (box_pos.y + box_size.y < ray_point.y) return false;
	if (box_pos.z > ray_point.z)  return false;
	if (box_pos.z + box_size.z < ray_point.z) return false;
